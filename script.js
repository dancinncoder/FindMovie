...
// --------------TMDB API: API request, get, show it on screen ----------------- //
...
function fetchSecondThen(data) {
  ...
  dataIndex.forEach((i) => { // i 는 index. 데이터모음집인 dataIndex에 있는 i번째 index 배열 요소 하나하나를 다 돈다.
    ...
    let temp_html = `        
      <div class="movie_card" data-id="${myId}">
        <img src="https://image.tmdb.org/t/p/w500${myPosterPath}" alt="">
        <div class="title_box">
          <h3>${myTitle}</h3>
          <p>Rating :  ${myVoteAverage}</p>
        </div>
        <p>${myOverView}</p>
  ...
  // listArea.forEach(card => {
  //     card.addEventListener("click", funtion () {
  //       let movieCardId = this.getAttribute("data-id");
  //       alert(`Movie ID : $(movieCardID)`);
  //     })
  // })
  ...
  function movieSearch(event) {
    ...
    const searchInput = document.getElementById('search-input'); // 검색엔진input박스를 Id를 통해 찾아라(선택)
